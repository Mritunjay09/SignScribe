  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      ...
      const newUser = await pool.query(
        'INSERT INTO users (name, email, google_id, password, profile_picture, role) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
        [
          profile.displayName,
...
passport.use(new FacebookStrategy(
  ...
  async (accessToken, refreshToken, profile, done) => {
    try {
      // Check if user already exists
      const existingUser = await pool.query('SELECT * FROM users WHERE facebook_id = $1 OR email = $2', 
        [profile.id, profile.emails?.[0].value]);
      
      if (existingUser.rows.length > 0) {
        // Update facebookId if user exists with same email
        if (!existingUser.rows[0].facebook_id) {
          await pool.query('UPDATE users SET facebook_id = $1 WHERE id = $2', 
            [profile.id, existingUser.rows[0].id]);
        }
        return done(null, existingUser.rows[0]);
      }
      ...
      // Create new user
      const newUser = await pool.query(
        'INSERT INTO users (name, email, facebook_id, password, profile_picture, role) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
        [
          profile.displayName,
          profile.emails?.[0].value || '',
          profile.id,
